...
import { asynccurrentUser, asyncaddedu, asynceditedu } from '../store/Actions/userActions'



const educationForm = ({ educationToEdit, closeModal }) => {
    ...
    useEffect(() => {
      ...
      if (educationToEdit) {
        setcollege(educationToEdit.college || "");
        setstartyear(educationToEdit.startyear || "");
        setendyear(educationToEdit.endyear || "");
        setdegree(educationToEdit.degree || "");
        setstream(educationToEdit.stream || "");
        setperformancescale(educationToEdit.performancescale || "");
        setperformance(educationToEdit.performance || "");
      }
    }, [educationToEdit]);
    ...
    const resumeSubmit = (e) => {
        ...
        // dispatch(asyncaddedu(newresume));
        // console.log("here is new",newresume)

        try {
          if (educationToEdit) {
            closeModal();
           dispatch(asynceditedu(educationToEdit.id, newresume))
            // Editing an existing entry
            // Dispatch an action to update the entry
            // For example: dispatch(asyncEditEducation(educationToEdit.id, newResume));
          } else {
            // Adding a new entry
            closeModal();
             dispatch(asyncaddedu(newresume));
